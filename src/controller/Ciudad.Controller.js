...
CiudadesCtrl.crearCiudad = async(req, res)=>{
    ...
    const{nombre, correo, contrasena} = req.body //viene del formulario
    const NuevaCiudad = new Ciudad({nombre, correo, contrasena })
    const correoCiudad = await Ciudad.findOne({correo: correo})
    if(correoCiudad){
        res.json({mensaje: 'correo ya existe'})
    }else{
        ...
        res.json({mensaje: 'Welcome', id: NuevaCiudad._id, nombre: NuevaCiudad.nombre, token})
    }
}
...
CiudadesCtrl.login= async(req, res)=>{
    ...
    if(match){
        ...
        res.json({mensaje: 'Bienvenido', id: ciudad.id, nombre: ciudad.nombre, token})
    }else{
